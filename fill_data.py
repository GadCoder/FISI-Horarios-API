...
def insert_cursos_from_carrera(plan: str, carrera: str, info_json):
    for ciclo in info_json["data"]:
        numero_ciclo = ciclo["num_CICLO_ANO_ASIGNATURA"]
        print(f"Ciclo : {ciclo}")
        cursos = ciclo["asignatura"]
        for curso in cursos:
...
def insert_docente(codigo_docente: str, nombre_docente: str, apellido_paterno: str, apellido_materno: str):
    url = f"{URL}/docentes/create-docente/"
    data = {
...
def insert_docentes_from_carrera(plan: str, carrera: str, info_json):
    for ciclo in info_json["data"]:
...
def insert_seccion(codigo_curso: str, codigo_docente: str, codigo_seccion: str, numero_seccion: int, carrera: str, plan: str):
    url = f"{URL}/secciones/create-seccion/{codigo_curso}/{codigo_docente}"
    data = {
...
def insert_secciones_from_carrera(plan: str, carrera, info_json):
    for ciclo in info_json["data"]:
        for curso in ciclo["asignatura"]:
            codigo_curso = carrera[:4] + "-" + str(curso["codAsignatura"])
            for seccion in curso["seccion"]:
...
def insert_horario_for_seccion(hora_inicio: int, 
    ...
    url = f"{URL}/horario-seccion/create-horario-seccion/"
    data = {
...
def insert_horarios_from_secciones(plan: str, carrera, info_json):
    for ciclo in info_json["data"]:
        for curso in ciclo["asignatura"]:
            codigo_curso = carrera[:4] + "-" + str(curso["codAsignatura"])
            for seccion in curso["seccion"]:
...
def main():
    ...
    for plan in planes:
        ...
        for carrera in carreras:
            ...
            insert_cursos_from_carrera(plan, carrera, info_json)
            insert_docentes_from_carrera(plan, carrera, info_json)
            insert_secciones_from_carrera(plan, carrera, info_json)
            insert_horarios_from_secciones(plan, carrera, info_json)
